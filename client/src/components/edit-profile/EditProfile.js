...
class CreateProfile extends Component {
  ...
  componentWillReceiveProps(nextProps, nextContext) {
    ...
    // fetch it only once
    if (nextProps.profile.profile && !this.state.initProfileFetched) {
        ...
        .filter((key) => {
            if (key === 'social') {
              Object.keys(profile.social)
                .filter((key) => {
                    return this.setState({[key]: !isEmpty(profile.social[key]) ? profile.social[key] : ''});
                  }
                );
            } else {
              return this.setState({[key]: !isEmpty(profile[key]) ? profile[key] : ''});
            }
          }
        );
    }
